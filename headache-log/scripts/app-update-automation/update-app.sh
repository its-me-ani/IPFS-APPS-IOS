...
if [[ -n "$ICON_SRC" && -f "$ICON_SRC" ]]; then
  ...
  echo "  → Generating iOS and PWA icons…"
  python3 - "$ICON_SRC" "$PROJECT_ROOT" <<'PYEOF' || true
...
try:
    ...
    src_icon = sys.argv[1]
    project_root = sys.argv[2]
    with Image.open(src_icon) as im:
        ios_dir = os.path.join(project_root, "ios/App/App/Assets.xcassets/AppIcon.appiconset")
        if os.path.exists(ios_dir):
            rgb_im = im.convert('RGB')
            resized = rgb_im.resize((1024, 1024), Image.Resampling.LANCZOS)
            resized.save(os.path.join(ios_dir, 'AppIcon-512@2x.png'), 'PNG')
            print("  ✓ iOS Icon saved: AppIcon-512@2x.png")
        ...
        for sz in [64, 192, 512]:
            rgba_im.resize((sz, sz), Image.Resampling.LANCZOS).save(os.path.join(public_dir, f'pwa-{sz}x{sz}.png'), 'PNG')
        rgba_im.resize((48, 48), Image.Resampling.LANCZOS).save(os.path.join(public_dir, 'favicon.png'), 'PNG')
        print("  ✓ Web assets generated under public/ folder")
except Exception as e:
    print(f"  (Skipping PIL resize in bash: {e})")
PYEOF
  step "Generated iOS AppIcon and PWA icons"
