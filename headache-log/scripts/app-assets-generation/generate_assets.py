        ...
        for file in files:
            ...
            try:
                with Image.open(file_path) as im:
                    ...
                    if abs(width - height) <= 20 and min(width, height) >= 48:
                        ...
                        score = max(width, height) # Prioritize larger sizes
                        
                        # Boost scores for ideal names/paths
                        if 'whatsapp' in file.lower() or 'logo' in file.lower() or 'icon' in file.lower() or 'bi.png' in file.lower():
                            score += 10000
...
def process_branding(root_dir):
    ...
    # Check data.json for explicit sourceIconPath override
    data_json_path = os.path.join(root_dir, 'scripts/app-update-automation/data.json')
    if os.path.exists(data_json_path):
        try:
            with open(data_json_path, 'r', encoding='utf-8') as f:
                data_cfg = json.load(f)
                icon_path_cfg = data_cfg.get('icon', {}).get('sourceIconPath', '')
                if icon_path_cfg:
                    full_icon_path = icon_path_cfg if os.path.isabs(icon_path_cfg) else os.path.join(root_dir, icon_path_cfg)
                    if os.path.exists(full_icon_path):
                        source_icon = full_icon_path
                        print(f"🎯 Using explicit data.json Source Icon: {os.path.relpath(source_icon, root_dir)}")
        except Exception as e:
            print(f"Warning reading data.json icon path: {e}")
